...
/**
 * Application service for user-related use cases (register, login, profile).
 */
public class UserService {
    ...
    /**
     * Computes rating statistics for a user.
     * Used for profile information (e.g., total ratings and average score).
     */
    public UserRatingStats getUserRatingStats(UUID userId) {
        // Note: ratings.listByUser(userId) should return an empty list if user has no ratings
     ...
     * - password is stored as a hash (never stored in plain text)
     */
    ...
    /**
     * Logs a user in and returns an authentication token (Bearer token).
     * <p>
     * Security:
    ...
    public String login(String username, String rawPassword) {
        ...
        // Delegates token creation to the token service (port)
        return tokens.issueToken(u.getId(), u.getUsername());
    }
    /**
     * Returns the user profile information for a given user ID.
     */
